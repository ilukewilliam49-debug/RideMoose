...
function ReceiptBreakdown({ receipt }: { receipt: FareReceipt }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2 mt-4 p-4 rounded-lg bg-secondary/50 border border-border">
      <h3 className="text-sm font-semibold text-primary">{t("meter.fareBreakdown")}</h3>
      <div className="grid grid-cols-2 gap-1 text-xs">
        <span className="text-muted-foreground">{t("meter.baseFare")}</span>
        <span className="text-right font-mono">{cents(receipt.baseFare)}</span>
        <span className="text-muted-foreground">{t("meter.distance")} ({receipt.distanceKm.toFixed(2)} km)</span>
        <span className="text-right font-mono">{cents(receipt.distanceCharge)}</span>
        <span className="text-muted-foreground">
          {t("meter.waiting")} ({receipt.totalWaitingMin.toFixed(1)} min, {receipt.freeWaitingMin} {t("meter.free")})
        </span>
        ...
        {receipt.billableWaitingMin > 0 && (
          <>
            <span className="text-muted-foreground text-[10px] pl-2">
              {t("meter.billable")}: {receipt.billableWaitingMin.toFixed(1)} min
            </span>
            <span />
          </>
        )}
          ...
          <div className="text-center mb-4">
            <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">
              {state.status === "running"
                ? state.isWaiting
                  ? t("meter.meterRunningWaiting")
                  : t("meter.meterRunning")
                : t("meter.finalFare")}
            </p>
            <motion.p
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            <div className="space-y-0.5">
              <div className="flex items-center justify-center gap-1 text-muted-foreground">
                ...
                <span className="text-[10px] uppercase">{t("meter.distance")}</span>
              </div>
            ...
            <div className="space-y-0.5">
              <div className="flex items-center justify-center gap-1 text-muted-foreground">
                ...
                <span className="text-[10px] uppercase">{t("meter.waiting")}</span>
              </div>
      ...
      <div className="flex gap-2">
        {state.status === "idle" && (
          <Button onClick={startMeter} className="flex-1 gap-2">
            <Play className="h-4 w-4" /> {t("meter.startMeter")}
          </Button>
        )}
        {state.status === "running" && (
          <>
            ...
            >
              {state.isWaiting ? (
                <><Play className="h-4 w-4" /> {t("meter.resume")}</>
              ) : (
                <><Pause className="h-4 w-4" /> {t("meter.waitingBtn")}</>
              )}
            </Button>
            <Button onClick={endMeter} variant="destructive" className="flex-1 gap-2">
              <Square className="h-4 w-4" /> {t("meter.endRide")}
            </Button>
          </>
        )}
      </div>
